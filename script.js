...
class Chrosmosome {
  ...
  setFitness() {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      for (let j = 0; j < isi[i].length; j++) {
        let harga =
          items[isi[i][j]].berat * dijkstraMap[0][items[isi[i][j]].kota_tujuan];
        if (items[isi[i][j]].getVolume() <= Chrosmosome.batasDimensiKecil) {
          harga *= Chrosmosome.hargaKecil;
        } else if (
          items[isi[i][j]].getVolume() <= Chrosmosome.batasDimensiMenengah
        ) {
    ...
    this.fitness -=
      jarak *
      (Chrosmosome.upahPerKM +
        Chrosmosome.rasioBensinPerKM * Chrosmosome.hargaBensin);
  }
}
...
for (let gen = 0; gen < 3; gen++) {
  ...
  new_chrosmosomes.push(
    new Chrosmosome(chrosmosomes[chrosmosomes.length - 1].data)
  );

  while (new_chrosmosomes.length < chrosmosomes.length) {
    let first_pick, second_pick;

    first_pick = randomInteger(
      0,
      (chrosmosomes.length * (chrosmosomes.length + 1)) / 2.0
    );
    ...
    for (let i = 0; i < offspring_count; i++) {
      let offspring = uniformCrossover(
