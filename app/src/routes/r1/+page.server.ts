...
export const actions: Actions = {
	...
	createBarang: async ({ request }) => {
		const formData = Object.fromEntries(await request.formData());
		const { Width, Height, Depth, OriginCity, DestCity, MustDeliver } = formData as {
			[key: string]: string;
		};
	...
	updateBarang: async ({ request }) => {
		const formData = Object.fromEntries(await request.formData());
		const { id, Width, Height, Depth, OriginCity, DestCity, MustDeliver } = formData as {
			[key: string]: string;
		};
	...
	createTruck: async ({ request }) => {
		const formData = Object.fromEntries(await request.formData());
		const { Width, Height, Depth, Weight, pf, ppk, cpk, ppl } = formData as {
			[key: string]: string;
		};
	...
	updateTruck: async ({ request }) => {
		const formData = Object.fromEntries(await request.formData());
		const { id, Width, Height, Depth, Weight, pf, ppk, cpk, ppl } = formData as {
			[key: string]: string;
		};
